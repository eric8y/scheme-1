...
def do_lambda_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    if len(vals)>2:
        return Pair('lambda', Pair(formals,Pair(Pair('begin',vals.second),nil)))
    return Pair('lambda',Pair(formals,Pair(vals[1],nil)))
...
def do_begin_form(vals, env):
    ...
    check_form(vals, 1)
    scheme_eval(vals, env)
    "*** YOUR CODE HERE ***"
    for elem in range(len(vals) - 1):
        return scheme_eval(vals[elem], env)
